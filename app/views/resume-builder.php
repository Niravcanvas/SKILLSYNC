...
document.addEventListener("DOMContentLoaded", function () {
  ...
  refreshBtn?.addEventListener("click", function () {
    ...
    fetch("../../app/controllers/groq_resume.php", { method: "POST", body: fd })
      .then(r => { if (!r.ok) throw new Error(); return r.json(); })
      .then(data => {
        if (data.success && data.career_objective) {
          ...
          toast("✨ Career objective generated by AI!", "success");
        } else if (data.error) {
          toast(data.error, "error");
        }
      })
      .catch(() => toast("AI generation failed. Please try again.", "error"))
      .finally(() => {
